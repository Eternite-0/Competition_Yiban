...
const calendarEvents: Record<number, { label: string; tone: 'primary' | 'warning' | 'error' }> = {
  7: { label: '蓝桥杯报名截止', tone: 'primary' },
  15: { label: '数模国赛初赛', tone: 'error' },
  26: { label: '省赛开幕', tone: 'warning' },
};
...
export default function StudentHome() {
  ...
  const [currentMonth] = useState({ year: 2026, month: 4 });
  ...
  const stats = [
    { label: '近期可报名', value: competitions.length, hint: '本周新增 3 项', icon: 'emoji_events' },
    { label: '已报名赛事', value: registeredCount, hint: `进行中 ${Math.min(registeredCount, 2)}`, icon: 'assignment_ind' },
        ...
        >
          <div className="flex items-center justify-between mb-lg">
            ...
            <div className="flex items-center gap-1 text-[14px] text-ink-muted-80">
              <button className="w-8 h-8 grid place-items-center rounded-full hover:bg-primary/6 transition">
